# README for Task 12: My Own Plugin
...
* A simple subsystem with a single timer that can with Start / Stop / Reset /
Countdown. It also comes with Default Widget and sounds. There is a whole README
file for it. in `Plugins/TimerPlugin` directory.
* Usage: You can check out how it is used in `L_Act3`. Open `L_MainLevel` and go
to "Act 3". Behind the last house there is a lever, that starts timer for 16
seconds and turns the platforms and rotating wheel on. After time is out, all
the mechanisms stop.

### Plugin created by someone else

I added to the project a very simple plugin from FAB.

* Plugin: LookAtActor – [LAA by Cyrus 365](https://www.fab.com/listings/d1b2e393-76f6-4651-8e24-5cab7dde112d)
* Description: A single Actor Component that handles constantly rotating one
actor towards the other.
* Usage: I've added it to some NPCs to always rotate towards the Player
Character. E.g. an NPC at near player when starting `L_MainLevel`.
